...
public abstract class RedstoneEtherAddons {
    ...
    public static void loadServerWorld() {
        if (serverManager == null) serverManager = new RedstoneEtherServerAddons();
    }
    ...
    public static String localizeWirelessItem(String shortName, int freq) {
        if (freq <= 0 || freq > RedstoneEther.numfreqs || RedstoneEther.client() == null) return shortName;

        String freqname = RedstoneEther.get(true).getFreqName(freq);
        if (freqname != null && !freqname.equals("")) return freqname;
        if (RedstoneEther.get(true).getFreqColourId(freq) == -1) return shortName + ' ' + freq;
