...
public class GuiPrivateSniffer extends GuiWirelessSniffer {
    ...
    protected void mouseClicked(int mousex, int mousey, int button) {
        ...
        if (RedstoneEther.get(true).canBroadcastOnFrequency(mc.thePlayer, freq)
                && freq > RedstoneEther.get(true).getLastPublicFrequency()
                && freq <= RedstoneEther.get(true).getLastSharedFrequency()) {
    ...
    public String getFreqTip(int freq) {
        if (RedstoneEther.get(true).isPlayerJammed(mc.thePlayer)) return "Jammed " + freq;
        ...
        if (freq <= RedstoneEther.get(true).getLastPublicFrequency()) return "Public " + freq;

        if (freq <= RedstoneEther.get(true).getLastSharedFrequency()) return "Shared " + freq;
    ...
    public Colour getColour(int freq) {
        ...
        if (freq <= RedstoneEther.get(true).getLastPublicFrequency()) colour.interpolate(colourJammed, 0.5);
    ...
    public Colour getBorder(int freq) {
        ...
        else border = borderOff.copy().interpolate(borderOn, brightness[freq - 1] / 64F);

        if (freq <= RedstoneEther.get(true).getLastPublicFrequency()) border.interpolate(borderJammed, 0.7);
