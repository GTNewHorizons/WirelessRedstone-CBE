...
public class TriangTexManager {
    ...
    private static int allocateSlot(int damage) {
        if (freeslots.isEmpty()) throw new RuntimeException("More than 256 different triangulators!");
    ...
    private static void mergeTexturesWithColour(ColourARGB texcolour) {
        for (int i = 0; i < 256; i++) {
            Colour colour;
            if (texGrad[i].a == 0) colour = texRing[i];
            else colour = texGrad[i].copy().multiply(texcolour);
