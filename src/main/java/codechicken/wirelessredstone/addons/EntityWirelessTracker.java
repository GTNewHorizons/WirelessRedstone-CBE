...
public class EntityWirelessTracker extends Entity implements WirelessTransmittingDevice {
    ...
    public void onEntityUpdate() {
        ...
        if (attached && attachedEntity == null) {
            if (!worldObj.isRemote) findAttachedEntity();
        } else if (isAttachedToEntity()) {
            trackEntity();

            if (!worldObj.isRemote) checkDetachment();
        } else {
            applyPhysics();
            moveEntityWithBounce(1);
            if (!worldObj.isRemote) attachToNearbyEntities();
        }

        if (item && attachmentCounter == 0) {
            item = false;
            if (!worldObj.isRemote) RedstoneEtherAddons.server().updateTracker(this);
        }
        if (isBurning()) {
            extinguish();
            if (!worldObj.isRemote) RedstoneEtherAddons.server().updateTracker(this);
        ...
        if (!attachedToLogout()) attachmentCounter--;
    }
    ...
    public boolean isRetractingStickyPistonFacing(int x, int y, int z, int facing) {
        ...
        if (block != Blocks.piston_extension) return false;
    ...
    public void attachToEntity(Vector3 pos, Entity e) {
        ...
        if (attachedEntity instanceof EntityPlayer) attachedPlayerName = attachedEntity.getCommandSenderName();
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (attached) attachmentCounter = 5;
    }
    ...
    public float getEntityRotation() {
        if (attachedEntity instanceof EntityLivingBase) return -((EntityLivingBase) attachedEntity).renderYawOffset;
    ...
    public EntityLivingBase getAttachedEntity() {
        if (attachedEntity instanceof EntityLivingBase) return (EntityLivingBase) attachedEntity;
        return null;
    }
