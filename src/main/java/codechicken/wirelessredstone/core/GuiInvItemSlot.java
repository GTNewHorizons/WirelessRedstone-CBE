...
public class GuiInvItemSlot extends GuiWidget {
    ...
    public void keyTyped(char c, int keyindex) {
        ...
        if (keyindex == Keyboard.KEY_LEFT) cyclePrevItem();
        if (keyindex == Keyboard.KEY_RIGHT) cycleNextItem();
        if (keyindex == Keyboard.KEY_RETURN && actionCommand != null) sendAction(actionCommand);
    }
    ...
    public void decrementCurrentStack() {
        ...
        if (inv.player.worldObj.isRemote) WRCoreCPH.sendDecrementSlot(slot);
    ...
    public void selectItem(int index) {
        selecteditem = index;
        if (invitems[selecteditem] == null) cycleNextItem();
    }
    ...
    public void cycleNextItem() {
        ...
        while (true) {
            cycleindex++;
            if (cycleindex >= invitems.length) cycleindex = 0;
            if (cycleindex == selecteditem) return;
    ...
    public void cyclePrevItem() {
        ...
        while (true) {
            cycleindex--;
            if (cycleindex < 0) cycleindex = invitems.length - 1;
            if (cycleindex == selecteditem) return;
