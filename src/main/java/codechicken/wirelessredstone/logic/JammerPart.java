...
public class JammerPart extends WirelessPart implements ITileJammer {
    ...
    public void onWorldJoin() {
        if (!world().isRemote && active()) addToEther();
    }

    @Override
    public void onWorldSeparate() {
        if (!world().isRemote && active()) removeFromEther();
    }
    ...
    public void addToEther() {
        if (active()) RedstoneEther.server().addJammer(world(), x(), y(), z());
    }
